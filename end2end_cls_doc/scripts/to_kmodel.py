...
def to_kmodel(onnx_model_path, kmodel_path, img_size, mean, std, input_range, samples_txt_path, tmp_path, ptq_option,
              target='k230'):
    ...
    img_paths = read_txt_as_list(samples_txt_path)
    ...
    ptq_options.set_tensor_data(
        generate_data_by_val([1, 3, img_size[1], img_size[0]], ptq_options.samples_count, samples_txt_path))
...
def generate_data_by_val(shape, batch, samples_txt_path):
    img_paths = read_txt_as_list(samples_txt_path)
    data = []
    for i in range(batch):
        assert i < len(img_paths), "calibration images not enough."
...
if __name__ == "__main__":
    to_kmodel('../checkpoints/best.onnx',
              '../checkpoints/best.kmodel', [224, 224],
              [0.485, 0.456, 0.406], [0.229, 0.224, 0.225], [0, 1], samples_txt_path='../gen/samples.txt',
