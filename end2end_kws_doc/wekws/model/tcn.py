...
class Block(nn.Module):
    ...
    def forward(
        ...
        if torch.tensor(cache.size(0), dtype=torch.int32) == 0:
            y = F.pad(x, (self.padding, 0), value=0.0)
        else:
            y = torch.cat((cache, x), dim=2)
        assert torch.tensor(y.size(2), dtype=torch.int32) > self.padding
...
class TCN(nn.Module):
    ...
    def forward(
        ...
        for block in self.network:
            if torch.tensor(in_cache.size(0), dtype=torch.int32) > 0:
