...
my_dataset_dir=$data_dir/$my_keyword
audio_dir=$my_dataset_dir/audio
my_keyword_file=$audio_dir/$my_keyword
...
unzip $my_file_name -d $audio_dir
...
# 将长音频分割为一个个正样本
python local/split_wav.py --folder_path $audio_dir/wav_from_k230 --output_folder $my_keyword_file
rm -r $audio_dir/wav_from_k230
# 将自己的音频样本添加到speech_commands数据集中的validation_list.txt和testing_list.txt中
python local/make_label_list.py --folder_path $my_keyword_file --val_txt $audio_dir/validation_list.txt --test_txt $audio_dir/testing_list.txt --my_keyword $my_keyword
exit 0
