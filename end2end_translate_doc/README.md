...
#### k230_code/sentencepiece/build.sh

`sentencepiece`编译脚本如下：
...
将项目中的k230_code文件夹拷贝到k230_sdk目录下的`src/big/nncase`下，进入`k230_code/sentencepiece`文件夹下，执行编译脚本，编译得到build文件夹，在`src/big/nncase/riscv64`下建立`sentencepiece`文件夹，将编译得到的`src/big/nncase/k230_code/sentencepiece/build`文件夹下的`lib`，`include`，`bin`文件夹拷贝到`src/big/nncase/riscv64/sentencepiece/`下。
进入`src/big/nncase/k230_code`下，执行编译脚本，将C++代码编译成`main.elf`可执行文件。如果编译可以在CANMV-K230开发板执行的elf文件：
